...
class AdminXmlImportControllerCore extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->separator = ($separator = Tools::substr((string) trim(""), 0, 1)) ? $separator : ';';
        $this->convert = false;
    ...
    public function ajaxProcessuploadXml()
    {
        ...
        if (isset($_FILES['file']) && !empty($_FILES['file']['error'])) {
            ...
            $_FILES['file']['error'] = $this->trans('The extension of your file should be ".xml".', [], 'Admin.Advparameters.Notification');
    ...
    protected function generateContentTable($current_table, $nb_column, $handle, $glue)
    {
        ...
        for ($current_line = 0; $current_line < 10 && $line = simplexml_load_file($handle, MAX_LINE_SIZE, $glue); ++$current_line) {
            /* UTF-8 conversion */
            if ($this->convert) {
    ...
    protected static function split($field)
    {
        ...
        $tab = simplexml_load_file($fd, MAX_LINE_SIZE, $separator);
        fclose($fd);
    ...
    public function categoryImport($offset = false, $limit = false, &$crossStepsVariables = false, $validateOnly = false)
    {
        ...
        $line_count = 0;
        for ($current_line = 0; ($line = simplexml_load_file($handle, MAX_LINE_SIZE, $this->separator)) && (!$limit || $current_line < $limit); ++$current_line) {
            ++$line_count;
            if ($this->convert) {
                $line = $this->utf8EncodeArray($line);
            }
    ...
    protected function getNbrColumn($handle, $glue)
    {
        ...
        $tmp = simplexml_load_file($handle, MAX_LINE_SIZE, $glue);
        AdminImportController::rewindBomAware($handle);
    ...
    protected function openCsvFile($offset = false)
    {
        ...
        for ($i = 0; $i < $toSkip; ++$i) {
            $line = simplexml_load_file($handle, MAX_LINE_SIZE, $this->separator);
            if ($line === false) {
    ...
    protected function excelToCsvFile($filename)
    {
        if (preg_match('#(.*?)\.(csv)#is', $filename)) {
            ...
            $csv_name = basename($filename, '.' . $info['extension']) . '.xml';
        ...
        if (Tools::getValue('csv')) {
            ...
            if ($results !== null) {
                ...
                if ($offset === 0) {
                    ...
                    if ($handle) {
                        $count = 0;
                        while (simplexml_load_file($handle, MAX_LINE_SIZE, $this->separator)) {
                            ++$count;
                        }
